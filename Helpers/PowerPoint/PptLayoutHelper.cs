...
public static class PptLayoutHelper
{
    ...
    public static List<GetLayoutInfo> BuildLayoutsList(IMasterLayoutSlideCollection layoutSlides)
    {
        List<GetLayoutInfo> layoutsList = [];
        for (var i = 0; i < layoutSlides.Count; i++)
        {
            var layout = layoutSlides[i];
            layoutsList.Add(new GetLayoutInfo
            {
                Index = i,
                Name = layout.Name,
                Type = layout.LayoutType.ToString()
            });
        }
