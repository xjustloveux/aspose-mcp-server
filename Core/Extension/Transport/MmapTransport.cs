...
public class MmapTransport : IExtensionTransport, IDisposable
{
    ...
    private MmapEntry CreateFileBackedMmap(string mmapName, int dataLength)
    {
        ...
        try
        {
            // T12: resolve immediately before FileStream creation to catch a symlink planted
            // inside _backingFileDirectory (bug 20260415-symlink-toctou-sweep, Phase 1).
            SecurityHelper.ResolveAndEnsureWithinAllowlist(
                filePath, [_backingFileDirectory], "filePath");
    ...
    /// </summary>
    /// <param name="filePath">The path of the file to delete. Null or empty is a no-op.</param>
    /// <remarks>
    ///     Symlink resolution is skipped here because (a) this is an error-path cleanup for a file
    ///     that was just generated by this class, and (b) the static context lacks the
    ///     <c>_backingFileDirectory</c> allowedBases reference. The primary symlink protection for
    ///     this directory is applied at creation time (T12 in <c>CreateFileBackedMmap</c>).
    /// </remarks>
            ...
            if (!string.IsNullOrEmpty(_backingFileDirectory))
                try
                {
                    if (Directory.Exists(_backingFileDirectory))
                        // T14: SafeRecursiveDelete instead of Directory.Delete(_, true) to avoid
                        // following child symlinks outside _backingFileDirectory
                        // (bug 20260415-symlink-toctou-sweep, Phase 1).
                        SecurityHelper.SafeRecursiveDelete(_backingFileDirectory, [_backingFileDirectory],
                            nameof(_backingFileDirectory));
                }
                catch (Exception ex)
                {
                    _logger?.LogDebug(ex,
        ...
        try
        {
            ...
            foreach (var dir in orphanDirs)
            {
                ...
                if (IsOrphanedDirectory(dir, pid))
                    ...
                    {
                        // T14: SafeRecursiveDelete instead of Directory.Delete(_, true) to avoid
                        // following child symlinks outside baseDirectory
                        // (bug 20260415-symlink-toctou-sweep, Phase 1).
                        SecurityHelper.SafeRecursiveDelete(dir, [baseDirectory], nameof(dir));
                        cleanedCount++;
                    }
                    catch (Exception ex)
                    {
