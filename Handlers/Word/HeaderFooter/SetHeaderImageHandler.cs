...
public class SetHeaderImageHandler : OperationHandlerBase<Document>
{
    ...
    /// <summary>
    ///     Sets an image in the document header.
    /// </summary>
    ...
    public override string Execute(OperationContext<Document> context, OperationParameters parameters)
    {
        ...
        foreach (var section in sections)
            InsertHeaderImage(section, doc, hfType, p);

        MarkModified(context);
        return Success($"Header image set{(p.IsFloating ? " (floating)" : "")}");
    }
    ...
    /// <summary>
    ///     Inserts an image into the header.
    /// </summary>
    ...
    private static void InsertHeaderImage(Section section, Document doc, HeaderFooterType hfType, ImageParameters p)
    {
        var header = WordHeaderFooterHelper.GetOrCreateHeaderFooter(section, doc, hfType);

        if (p.RemoveExisting)
            RemoveExistingImages(header);
        ...
        if (p.IsFloating)
            ApplyFloatingPosition(shape, section, p.Alignment, RelativeVerticalPosition.TopMargin);
        else
