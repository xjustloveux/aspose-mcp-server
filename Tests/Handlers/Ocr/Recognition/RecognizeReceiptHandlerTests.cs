...
/// <summary>
///     Unit tests for RecognizeReceiptHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class RecognizeReceiptHandlerTests : HandlerTestBase<AsposeOcr>
{
    private readonly RecognizeReceiptHandler _handler = new();

    #region Operation Property

    [Fact]
    public void Operation_Returns_RecognizeReceipt()
    {
        Assert.Equal("recognize_receipt", _handler.Operation);
    }
    ...
    public void Execute_WithNonexistentFile_ShouldThrowFileNotFoundException()
    {
        ...
        {
            { "path", Path.Combine(TestDir, "nonexistent_receipt.jpg") }
        });
