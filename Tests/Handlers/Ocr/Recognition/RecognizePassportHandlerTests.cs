...
/// <summary>
///     Unit tests for RecognizePassportHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class RecognizePassportHandlerTests : HandlerTestBase<AsposeOcr>
{
    private readonly RecognizePassportHandler _handler = new();

    #region Operation Property

    [Fact]
    public void Operation_Returns_RecognizePassport()
    {
        Assert.Equal("recognize_passport", _handler.Operation);
    }
    ...
    public void Execute_WithNonexistentFile_ShouldThrowFileNotFoundException()
    {
        ...
        {
            { "path", Path.Combine(TestDir, "nonexistent_passport.jpg") }
        });
