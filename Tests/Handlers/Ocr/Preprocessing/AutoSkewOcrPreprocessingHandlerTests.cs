...
/// <summary>
///     Unit tests for AutoSkewOcrPreprocessingHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class AutoSkewOcrPreprocessingHandlerTests : OcrPreprocessingHandlerTestBase
{
    private readonly AutoSkewOcrPreprocessingHandler _handler = new();

    /// <inheritdoc />
    protected override IOperationHandler<AsposeOcr> Handler => _handler;

    /// <inheritdoc />
    protected override string ExpectedOperation => "auto_skew";

    #region Operation Property

    [Fact]
    public void Operation_Returns_AutoSkew()
    {
        Assert.Equal("auto_skew", _handler.Operation);
    }
    ...
    public void Execute_WithValidImage_ShouldReturnPreprocessingResult()
    {
        ...
        var outputPath = Path.Combine(TestDir, $"auto_skew_output_{Guid.NewGuid()}.bmp");
        var context = CreateContext(new AsposeOcr());
        var parameters = CreatePreprocessingParameters(tempImage, outputPath);

        var result = _handler.Execute(context, parameters);

        AssertPreprocessingResult(result, tempImage, outputPath, "auto_skew");
