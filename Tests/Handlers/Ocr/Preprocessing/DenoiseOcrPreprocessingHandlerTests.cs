...
/// <summary>
///     Unit tests for DenoiseOcrPreprocessingHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class DenoiseOcrPreprocessingHandlerTests : OcrPreprocessingHandlerTestBase
{
    private readonly DenoiseOcrPreprocessingHandler _handler = new();

    /// <inheritdoc />
    protected override IOperationHandler<AsposeOcr> Handler => _handler;

    /// <inheritdoc />
    protected override string ExpectedOperation => "denoise";

    #region Operation Property

    [Fact]
    public void Operation_Returns_Denoise()
    {
        Assert.Equal("denoise", _handler.Operation);
    }
    ...
    public void Execute_WithValidImage_ShouldReturnPreprocessingResult()
    {
        ...
        var outputPath = Path.Combine(TestDir, $"denoise_output_{Guid.NewGuid()}.bmp");
        var context = CreateContext(new AsposeOcr());
        var parameters = CreatePreprocessingParameters(tempImage, outputPath);

        var result = _handler.Execute(context, parameters);

        AssertPreprocessingResult(result, tempImage, outputPath, "denoise");
