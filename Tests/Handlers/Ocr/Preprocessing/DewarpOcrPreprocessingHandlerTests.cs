...
/// <summary>
///     Unit tests for DewarpOcrPreprocessingHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class DewarpOcrPreprocessingHandlerTests : OcrPreprocessingHandlerTestBase
{
    private readonly DewarpOcrPreprocessingHandler _handler = new();

    /// <inheritdoc />
    protected override IOperationHandler<AsposeOcr> Handler => _handler;

    /// <inheritdoc />
    protected override string ExpectedOperation => "dewarp";

    #region Operation Property

    [Fact]
    public void Operation_Returns_Dewarp()
    {
        Assert.Equal("dewarp", _handler.Operation);
    }
    ...
    public void Execute_WithValidImage_ShouldReturnPreprocessingResult()
    {
        ...
        var outputPath = Path.Combine(TestDir, $"dewarp_output_{Guid.NewGuid()}.bmp");
        var context = CreateContext(new AsposeOcr());
        var parameters = CreatePreprocessingParameters(tempImage, outputPath);

        var result = _handler.Execute(context, parameters);

        AssertPreprocessingResult(result, tempImage, outputPath, "dewarp");
