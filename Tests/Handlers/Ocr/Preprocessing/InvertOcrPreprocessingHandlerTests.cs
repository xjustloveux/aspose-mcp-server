...
/// <summary>
///     Unit tests for InvertOcrPreprocessingHandler.
///     Tests operation property, parameter validation, error handling, and happy path.
/// </summary>
public class InvertOcrPreprocessingHandlerTests : OcrPreprocessingHandlerTestBase
{
    private readonly InvertOcrPreprocessingHandler _handler = new();

    /// <inheritdoc />
    protected override IOperationHandler<AsposeOcr> Handler => _handler;

    /// <inheritdoc />
    protected override string ExpectedOperation => "invert";

    #region Operation Property

    [Fact]
    public void Operation_Returns_Invert()
    {
        Assert.Equal("invert", _handler.Operation);
    }
    ...
    public void Execute_WithValidImage_ShouldReturnPreprocessingResult()
    {
        ...
        var outputPath = Path.Combine(TestDir, $"invert_output_{Guid.NewGuid()}.bmp");
        var context = CreateContext(new AsposeOcr());
        var parameters = CreatePreprocessingParameters(tempImage, outputPath);

        var result = _handler.Execute(context, parameters);

        AssertPreprocessingResult(result, tempImage, outputPath, "invert");
