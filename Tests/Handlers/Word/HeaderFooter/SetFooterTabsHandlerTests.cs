...
public class SetFooterTabsHandlerTests : WordHandlerTestBase
{
    ...
    [Fact]
    public void Execute_WithNoTabStops_SetsFooter()
    {
        var doc = CreateEmptyDocument();
        var context = CreateContext(doc);
    ...
    [Fact]
    public void Execute_WithTabStops_SetsFooterTabs()
    {
        var doc = CreateEmptyDocument();
        var context = CreateContext(doc);
        var tabStops = new JsonArray
    ...
    public void Execute_WithAMalformedEntryLaterInTheArray_LeavesTheExistingStops()
    {
        ...
        // a bad entry threw after Clear(), so the footer came back with no stops at all and the
        ...
        var stops = doc.FirstSection.HeadersFooters[HeaderFooterType.FooterPrimary]!
