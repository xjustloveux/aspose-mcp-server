...
public class OutputSchemaValidationTests
{
    ...
    ///     Schemas generated by <c>GenerateFromNamespace</c> are wrapped in a top-level
    ///     <c>result</c> property to match the MCP SDK 1.2.0 wire format; this helper
    ///     transparently unwraps that layer before validating the inner structure so the
    ///     same assertions apply to both namespace-generated and type-generated schemas.
    /// </summary>
    ...
    private static void ValidateSchemaStructure(string schemaJson, string toolName, List<string> failures)
    {
        try
        {
            ...
            var topProperties = schemaNode["properties"]!.AsObject();

            // Namespace-generated schemas wrap the FinalizedResult shape under a top-level
            // "result" property to align with the SDK's auto-wrap of object-returning tools.
            // Drill into that wrapper to validate the inner {data, output} shape.
            JsonObject innerSchema;
            JsonObject innerProperties;
            if (topProperties.ContainsKey("result"))
            {
                innerSchema = topProperties["result"]!.AsObject();
                innerProperties = innerSchema["properties"]!.AsObject();
            }
            else
            {
                innerSchema = schemaNode;
                innerProperties = topProperties;
            }

            if (!innerProperties.ContainsKey("data"))
                failures.Add($"{toolName}: Schema missing 'data' field");

            if (!innerProperties.ContainsKey("output"))
                failures.Add($"{toolName}: Schema missing 'output' field");

            if (innerSchema.ContainsKey("required"))
            {
                var required = innerSchema["required"]!.AsArray();
            ...
            if (innerProperties.ContainsKey("output"))
            {
                var outputSchema = innerProperties["output"]!.AsObject();
