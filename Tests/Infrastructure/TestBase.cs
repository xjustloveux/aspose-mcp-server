...
public abstract class TestBase : IDisposable
{
    ...
    /// <summary>
    ///     Extracts the data element from a FinalizedResult structure as a strongly-typed object.
    /// </summary>
    /// <typeparam name="T">The expected data type.</typeparam>
    /// <param name="result">The tool result object (should be FinalizedResult&lt;T&gt;).</param>
    /// <returns>The strongly-typed data object.</returns>
    /// <exception cref="InvalidCastException">Thrown when result is not FinalizedResult&lt;T&gt;.</exception>
    protected static T GetResultData<T>(object result)
    {
        if (result is FinalizedResult<T> finalized)
            return finalized.Data;

        throw new InvalidCastException(
            $"Expected FinalizedResult<{typeof(T).Name}> but got {result.GetType().Name}. " +
            "This likely means the Tool or Handler hasn't been updated to use FinalizedResult.");
    }

    /// <summary>
    ///     Extracts the OutputInfo from a FinalizedResult structure.
    /// </summary>
    /// <typeparam name="T">The expected data type.</typeparam>
    /// <param name="result">The tool result object (should be FinalizedResult&lt;T&gt;).</param>
    /// <returns>The OutputInfo object containing path and session information.</returns>
    /// <exception cref="InvalidCastException">Thrown when result is not FinalizedResult&lt;T&gt;.</exception>
    protected static OutputInfo GetResultOutput<T>(object result)
    {
        if (result is FinalizedResult<T> finalized)
