...
public class OcrRecognitionTool
{
    ...
    /// </param>
    /// <param name="targetFormat">Target format for recognize_pdf (docx, xlsx, pdf, txt).</param>
...
- Convert scanned PDF to DOCX: ocr_recognition(operation='recognize_pdf', path='scan.pdf', outputPath='output.docx', targetFormat='docx')
- Convert scanned PDF to searchable PDF: ocr_recognition(operation='recognize_pdf', path='scan.pdf', outputPath='searchable.pdf', targetFormat='pdf')

Supported image formats: PNG, JPG, BMP, TIFF, GIF
Supported output formats (recognize_pdf): docx, xlsx, pdf, txt

Note: OCR requires ONNX Runtime and is not supported on Linux ARM64.")]
    public object Execute(
        [Description(@"Operation to perform.
- 'recognize': Recognize text from an image or PDF file (required params: path)
- 'recognize_pdf': Convert scanned PDF to editable document (required params: path, outputPath, targetFormat)")]
        string operation,
        ...
        [Description(
            "Recognition language (default: Eng). Values: Eng, Chi, Deu, Fra, Spa, Ita, Por, Rus, Hin, or full names like English, Chinese, German")]
        string language = "Eng",
        [Description("Target format for recognize_pdf: docx, xlsx, pdf, txt")]
        ...
        bool includeWords = false)
    {
        ...
        var parameters = BuildParameters(path, outputPath, language, targetFormat, includeWords);
