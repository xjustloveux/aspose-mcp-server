...
/// <summary>
///     MCP tool exposing PowerPoint-presentation OLE-object operations (<c>list</c> /
///     <c>extract</c> / <c>extract_all</c> / <c>remove</c>) over Aspose.Slides'
///     <see cref="Aspose.Slides.IOleObjectFrame" /> surface. Accepts <c>.pptx</c> and
///     <c>.ppt</c>.
/// </summary>
[ToolHandlerMapping("AsposeMcpServer.Handlers.PowerPoint.OleObject")]
[McpServerToolType]
public class PptOleObjectTool
{
    /// <summary>Registry for the four PowerPoint OLE handlers.</summary>
    ...
    /// <summary>Initializes a new instance of the <see cref="PptOleObjectTool" /> class.</summary>
    /// <param name="sessionManager">Optional session manager.</param>
    /// <param name="identityAccessor">Optional identity accessor.</param>
    /// <param name="serverConfig">Optional server config.</param>
    public PptOleObjectTool(
        ...
        ServerConfig? serverConfig = null)
    {
        ...
        _handlerRegistry = HandlerRegistry<Presentation>.CreateFromNamespace(
            "AsposeMcpServer.Handlers.PowerPoint.OleObject");
    }

    /// <summary>Executes a PowerPoint OLE-object operation.</summary>
    ...
    /// <param name="oleIndex">Zero-based OLE index (flat across slides).</param>
    ...
    [McpServerTool(
        Name = "ppt_ole_object",
        Title = "PowerPoint OLE Object Operations",
        Destructive = true,
        Idempotent = false,
        OpenWorld = false,
        ReadOnly = false,
    ...
    [Description(
        "Manage OLE objects embedded in PowerPoint presentations (.pptx, .ppt). Supports 4 operations: list, extract, extract_all, remove.\n\n" +
        "Indices are flat across all slides. Remove semantics: removing index N shifts indices > N down by one (AC-17). " +
        ...
        string? outputPath = null)
    {
        ...
        if (string.IsNullOrEmpty(sessionId) && !string.IsNullOrEmpty(path))
        {
            ...
            OleExtensionGuard.EnsurePptExtension(path);
        }
        ...
        var opCtx = new OperationContext<Presentation>
        {
