...
public class WordSetFooterTabStopsTool : IAsposeTool
{
    public string Description => "Set tab stops for footer paragraphs in a Word document (fine-grained control)";
    ...
    public object InputSchema => new
    {
        ...
        properties = new
        {
            ...
            {
                type = "array",
                description = "Tab stops array. Example: [{\"position\": 297.65, \"alignment\": \"Center\"}, {\"position\": 541.45, \"alignment\": \"Right\"}]. Set to empty array [] to remove all tab stops.",
                items = new
                {
                    type = "object",
                    properties = new
            ...
            {
                type = "number",
                description = "Paragraph index in footer (0-based). Default: 0 (first paragraph)"
            }
        },
    ...
    public async Task<string> ExecuteAsync(JsonObject? arguments)
    {
        ...
        foreach (Section section in sections)
        {
            ...
            var paragraphs = footer.GetChildNodes(NodeType.Paragraph, true).Cast<Paragraph>().ToList();
        ...
        return await Task.FromResult($"成功設定頁尾定位點（{tabCount} 個）於 {sectionsDesc}，段落 {paragraphIndex}");
    }
}
